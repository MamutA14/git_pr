...
esAxL2 phi = case phi of
  (Oimp(Oimp a (Oimp b g)) (Oimp (Oimp a' b')(Oimp a'' g')))-> (a == a') && (a == a'') && (b == b') && (g == g')
  otherwise -> False 

-- Función que nos dice si una formula de PLI cumple el Axioma 3
esAxL3 :: PLI -> Bool
esAxL3 phi = case phi of
  (Oimp (Oimp (Oimp a Bot) (Oimp b Bot)) (Oimp b' a')) -> (a == a') && (b == b')
  otherwise -> False 
...
checkPaso lprem lpp p = case p of
  (n, (phi, Prem)) -> (filter (==phi) lprem) /= []  -- Revisamos que phi sea parte de lprem
